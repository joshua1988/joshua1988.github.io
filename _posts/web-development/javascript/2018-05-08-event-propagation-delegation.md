...
브라우저에서 어떻게 특정 화면 요소의 이벤트를 감지하는지 그리고 그 이벤트를 어떻게 다른 화면 요소에 전파시키는지 같이 알아보겠습니다.
...
![event bubbling](/Users/gihyojoshuajang/Documents/Programming/TIL/javascript/images/event-bubble-capture/event-bubble.png)
...
![event-bubble-log](/Users/gihyojoshuajang/Documents/Programming/TIL/javascript/images/event-bubble-capture/event-bubble-log.png)

div 태그 한 개만 클릭 했을 뿐인데 왜 3개의 이벤트가 발생되는 걸까요? 그 이유는 브라우저가 이벤트를 감지하는 방식 때문입니다.

브라우저는 특정 화면 요소에서 이벤트가 발생하면 그 이벤트를 최상위에 있는 화면 요소까지 이벤트를 전파시킵니다.
...
<p class="notice">참고 : 이벤트 버블링은 아래에서 설명 드릴 이벤트 위임을 이해할 때 알아야 하는 중요한 개념이니 꼭 알고 계세요.</p>

## 이벤트 캡쳐 - Event Capture down
이벤트 캡쳐는 이벤트 버블링과 반대 방향으로 진행되는 이벤트 전파 방식입니다.

![event-capture](/Users/gihyojoshuajang/Documents/Programming/TIL/javascript/images/event-bubble-capture/event-capture.png)
...
`addEventListener()` API에서 옵션 객체로 `capture:true`를 설정해주면 됩니다.
그러면 해당 이벤트를 감지하기 위해 이벤트 버블링과 반대 방향으로 탐색합니다.

![event-capture](/Users/gihyojoshuajang/Documents/Programming/TIL/javascript/images/event-bubble-capture/event-capture-log.png)
...
이벤트 위임은 실제 바닐라 JS로 웹 앱을 구현할 때 많이 사용하게 되는 코딩 패턴입니다.
이벤트 위임을 한 문장으로 요약해보면 '하위 요소에 이벤트를 붙이지 않고 상위 요소에서 하위 요소의 이벤트들을 제어하는 방식'입니다.
...
![event-delegation-1](/Users/gihyojoshuajang/Documents/Programming/TIL/javascript/images/event-bubble-capture/event-delegation-1.gif)
...
![event-delegation-2](/Users/gihyojoshuajang/Documents/Programming/TIL/javascript/images/event-bubble-capture/event-delegation-2.gif)
...
이런식으로 매번 새롭게 추가된 리스트 아이템까지 클릭 이벤트 리스너를 일일이 달아줘야 할까요?

리스트 아이템이 많아지면 많아질수록 이벤트 리스너를 다는 작업 자체가 매우 번거로워집니다.
...
![event-delegation-3](/Users/gihyojoshuajang/Documents/Programming/TIL/javascript/images/event-bubble-capture/event-delegation-3.gif)

이젠 리스트 아이템을 새로 추가할 때 마다 클릭 이벤트를 안 달아도 되겠네요 :)

<p class="notice">위 코드는 현재 인풋 박스의 이벤트만 다루는 것이 아니라 label 태그의 이벤트도 감지합니다. event 객체를 이용하여 인풋 박스의 이벤트만 감감지할 수 있도록 구현해보세요.</p>

## 마무리
오늘은 브라우저가 어떻게 이벤트를 감지하고 그에 따라 우리는 어떻게 이벤트를 다뤄야 하는지에 대해 알아보았습니다.
위 내용은 어떤 프레임워크를 쓰느냐와 관계 없이 기본적인 브라우저의 이벤트 감지 방식이기 때문에 알아두시면 유용하겠네요 :)
