...
## 비동기 처리? 그게 뭔가요?
자바스크립트 비동기 처리란 특정 연산의 수행이 끝날 때까지 기다리지 않고
다른 코드를 먼저 수행하는 자바스크립트의 특성을 의미합니다.

## 비동기 처리 첫 번째 사례
비동기 처리의 가장 흔한 사례는 제이쿼리의 ajax입니다.
제이쿼리로 실제 웹 서비스를 개발할 때 ajax 통신을 빼놓을 수가 없습니다.
보통 화면에 표시할 이미지나 데이터를 서버에서 불러와 표시해야 하는데 이때
ajax 통신을 이용하여 해당 데이터를 서버로부터 가져올 수 있기 때문입니다.
...
function getData() {
	var tableData;
	$.get('http://test.com/products/1', function (response) {
		tableData = response;
	});
	return tableData;
}
...
여기서 `$.get()`이 ajax 통신을 하는 부분입니다. 'http://test.com/products/1'이라는 url로
...
받아온 데이터를 tableData라는 변수에 저장합니다. 그럼 이제 이 getData()를 호출하면 어떻게 될까요?
...
setTimeout() 역시 비동기 처리 방식으로 실행되기 때문에 3초를 기다렸다가 다음 코드를 수행하는 것이 아니라
일단 setTimout()을 실행하고 나서 바로 다음 코드인 'Hello Again'으로 넘어갔습니다.
...
콜백 함수의 동작 방식은 일종의 식당 자리 예약과 같습니다. 일반적으로 맛 집을 가면 사람이 많아 자리가 없습니다.
