...
5년차를 맞이하는 웹 개발자로서 항상 어떻게 성장해야 하는지 고민하고 있습니다. 올 한 해는 이번 블로그 글을 염두에 두고,
더 좋은 개발자, 더 좋은 동료가 되보려고 합니다.

아래 글은 "흔하지 않은, 좋은 웹 개발자가 되는 방법(How to become an uncommonly web developer)"이라는 미디엄 기사를 번역한 글입니다.
7년의 경력을 가진 비전공 웹 개발자가 겪은 경험과 생각을 글로 잘 풀어냈죠. 훌륭한 웹 개발자가 되려면 어떤 자질이 필요할까? 라고 했을 때 다들 코딩을 떠올리실텐데요.
...
## 1. 특정 기술을 숙련하는 것은 스프린트가 아니라 마라톤입니다.
...
그리고 아마 많은 스트레스를 가져다줄 거에요.

한 가지 확실한 건 당신이 더 웹 개발을 잘하고 싶고 배우고 싶다는 거에요.
**그렇지만, 지금 현재 위치를 현실적으로 인지하고, 성장하는 과정을 넓게 바라보세요.**
커리어를 단기적인 관점이 아니라 장기적인 관점에서 바라봐야 합니다. 너무 일찍 포기하시면 안돼요.

## 2. 누군가에게 도움을 요청하기 전에 최대한 스스로 먼저 시도해보세요.
고대에는 미지의 영역을 탐험하는 게 불안과 스트레스를 가져다줬습니다.
이와 비슷하게, 저는 제가 알지 못하는 것을 해결해야 할 때 이런 느낌을 받았죠.
...
결국에는 제가 이해할 수 있는 최대까지 알아보게 되었죠.
...
**일찌감치 포기하고, 다른 사람에게 내 일을 대신해달라고 말하기 전에 제가 할 수 있는 최선을 다했습니다.**
...
물론 물어봐서 금방 해결될 문제를 스스로 해결하려다가 몇 날 며칠을 보내는 것도 아깝습니다.
특정 문제에 너무 많은 시간을 소모하면 아무래도 효율이 떨어지니까요.

여기서부터는 여러분 스스로 밸런스를 맞추셔야 합니다. 어느 정도까지 스스로 알아보다가 물어볼 건지를요.
아마 몇 번 거치다가보면 아주 좋은 밸런스를 찾게 될 겁니다.

## 3. 호기심 가지기
제가 프로그래머로 처음 일을 시작했을 때, 매번 긍정적인 피드백을 받았고 새로운 정보들을 스펀지처럼 흡수했습니다.
어떻게 해야 하는지 아무것도 몰라도 무엇이든 해보려고 했죠. 그리고 정말 동작 원리가 궁금했었습니다.

**프로그램이 어떻게 동작하는지 알고 싶어 하는 마음은 모든 엔지니어들에게 있습니다.**
**당신의 호기심이 그 마음을 커지게 만드세요.**

그냥 빨리 일 처리하려고 어떻게 돌아가는지 확인도 안하는 것은 바보 같은 짓입니다.
**이게 왜 동작하는지를 아는 것은 매우 중요합니다.**

특정 문제를 해결하기 위해 스택 오버 플로우에서 돌아가는 코드를 찾았다고, 복붙으로 끝내면 안됩니다.
정말 빠듯한 마감기한이 아니라면, 시간을 좀 더 들여서 코드가 어떻게 돌아가는지 그리고 당신의 코드와 어떻게 통합되는지
한번 살펴보세요. 왜 당신의 기존 코드는 돌아가지 않았고, 지금은 왜 돌아가는지도요.

설령 100% 다 이해를 못했더라도 괜찮습니다. 그냥 전체적인 개념을 잡는데 노력해보세요.
이번에 다 이해 못해도, 다음 번 유사한 문제를 만났을 때는 더 이해하기 쉬울 겁니다.

## 4. 항상 성장하기
이 주제는 웹 개발뿐만 아니라 요리, 운전 등 다른 영역에도 적용됩니다.

#### 타산지석 (실수를 분석하고, 실수로부터 배우기)
특정 문제가 잘 안 풀릴 때는 잠시 뒤로 물러나서 관점을 바꿔보세요.
그리고 이런 질문을 해보는 건 어떨까요?
*"내가 무엇을 하고 싶었었는지?"*
*"그리고 왜 그게 잘 안되었는지?"*

만약 웹 사이트 코드를 잘못 건들였다면, 무슨 일이 일어날까요?
그리고 다음에는 이 문제를 어떻게 방지할 수 있을까요?

아래의 절차를 따라보세요.
- 뒤로 돌아가서 가장 바람직하지 않은 상황을 분석해보세요.
- 무슨 일이 일어난 건지 파악해보세요.
- 다음번에는 이 문제를 피할 수 있는 실행 절차를 세워보세요.

이건 꽤 시간이 걸립니다. 그리고, 솔직하게 모든 결정들에 대해서 돌아가 파악할 시간이 없죠.
**하지만, 실수에서 배우지 않고 계속 잘못 이해한다면 다시 그 실수를 저지르는 건 뻔한 사실입니다.**

꽤 어려운 작업이죠. 왜냐면 누구나 다 부정적인 경험을 한번 하고 나면 다시는 그 경험을 하고 싶지 않아 하니까요.

그렇다면 이걸 디버깅으로 보면 어떨까요? 코드에 반복적으로 같은 오류를 발생시키면 꽤 비효율적이자나요.
달리 말하면 그 코드에는 다시 오류가 발생하지 않게 개선할 여지가 남아 있다는 거에요.
...
문제가 생기는 원인들을 찾아보세요. 원인을 찾고 나면 금방 그 미스테리한 문제를 해결할 수 있을거에요.

#### 램프 고치기
더 쉽게 설명하기 위해 고장 난 램프를 고치는 작업을 비유로 들어보겠습니다.

램프를 고치려면 먼저 램프의 정상 동작이 어떤 건지 파악해야 합니다.
다 알고 있다시피 램프는 전원을 켜면 불이 들어와야 하죠.
...
4. 램프 스위치가 켜졌을 때, 램프ㅇ의 전구가 켜지는지 확인합니다. 전구는 제대로 켜지나요? 깨지진 않았죠?

이렇게 각 단계를 확인해보면 문제가 발생한 범위를 좁힐 수 있습니다.
이런 방식을 모든 문제에 적용할 수 있겠죠.
...
전 특정 문제에 몇 일, 몇 주를 쓴 적도 있었죠. 이게 바로 웹 개발자의 삶에서 최악인 부분입니다.

하지만 반대로 몇 번의 고군분투 후에 해결책을 찾아내면 놀라운 성취감을 느낍니다.
이 순간만큼에는 굉장한 자신감을 느끼죠. 의심할 여지없이 그 감정이 바로 웹 개발자가 가질 수 있는 최선의 삶입니다.
...
우리는 코드와 일하긴 하지만 궁극적으로는 사람들과 일합니다.
웹 사이트와 애플리케이션 모두 사람을 위해 만들어졌죠.
당신의 직업이 무엇이든 간에 당신은 고객, 손님, 동료와 일하게 됩니다.

**최고의 웹 개발자는 단지 코딩만 잘하는 게 아닙니다. 큰 그림을 잘 이해하고, 사람들과 잘 어울려 협업할 수 있죠.**

다음 2개의 팁을 숙달하기는 쉽지 않을 겁니다. 왜냐하면 기존과는 다른 종류의 사고방식을 요구하기 때문이에요.
...
#### 옳고 그름을 따지기 보다..
제 남편과 싸우고 나면, 우리 둘 사이는 금방 어색해집니다.
...
그때 바로 결론내려고 하지 말고, 그리고 그 사람을 비난하지도 마세요.
아마 언쟁 중에 그 사람이 한 행동이나 한 말은 고의가 아니었을 겁니다.

**그에게 대화를 해보려고 하고, 생각을 이해하려고 해보세요.**
...
제 개인적으로 수동적이지 않고 능동적으로 행동하는 것이
성공하는 사람들의 특징이라고 생각합니다.
